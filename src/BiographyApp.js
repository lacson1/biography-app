...
const BiographyApp = () => {
        ...
        const handlePinSetup = (e) => {
            ...
            if (newPin.length === 4 && /^\d{4}$/.test(newPin)) {
                setPin(newPin);
                localStorage.setItem('biographyPin', newPin);
                setIsAuthenticated(true);
                setShowPinSetup(false);
                setPinError('');
            } else {
        ...
        // PIN UI overlays
        if (showPinSetup && !isAuthenticated) {
            return ( <
                div className = "min-h-screen flex items-center justify-center bg-gradient-to-br from-amber-50 via-orange-50 to-yellow-50" >
                <
                form onSubmit = { handlePinSetup }
                className = "bg-white rounded-xl shadow-lg p-8 w-full max-w-xs text-center" >
                <
                h2 className = "text-2xl font-bold mb-4" > Set a 4 - digit PIN < /h2> <
                input type = "password"
                maxLength = { 4 }
                pattern = "\d{4}"
                value = { newPin }
                onChange = { e => setNewPin(e.target.value.replace(/[^0-9]/g, '')) }
                className = "w-full p-3 border-2 border-gray-200 rounded-lg text-lg text-center mb-4"
                placeholder = "••••"
                autoFocus /
                >
                {
                    ...
                    button type = "submit"
                    className = "w-full bg-amber-600 text-white py-2 rounded-lg font-semibold" > Set PIN < /button> < /
                    form > <
                    /div>
                );
            }
            if (pin && !isAuthenticated) {
                return ( <
                    ...
                    className = "bg-white rounded-xl shadow-lg p-8 w-full max-w-xs text-center" >
                    <
                    h2 className = "text-2xl font-bold mb-4" > Enter your 4 - digit PIN < /h2> <
                    input type = "password"
                    maxLength = { 4 }
                    pattern = "\d{4}"
                    value = { enteredPin }
                    ...
                    >
                    {
                        pinError && < p className = "text-red-500 mb-2" > { pinError } < /p>} <
                        button type = "submit"
                        className = "w-full bg-amber-600 text-white py-2 rounded-lg font-semibold" > Unlock < /button> < /
                        form > <
                        /div>
                    );
                }
            ...
            return ( <
                    ...
                    className = "p-2 rounded-lg bg-gray-200 text-gray-700 hover:bg-gray-300 ml-2" > Change PIN < /button>
                    ...
                    { /* PIN change modal */ } {
                        showPinChange && ( <
                            ...
                            >
                            {
                                ...
                                <
                                button type = "submit"
                                className = "flex-1 bg-amber-600 text-white py-2 rounded-lg font-semibold" > Save < /button> <
                                button type = "button"
                                onClick = {
                                    () => {
                                        setShowPinChange(false);
                                ...
                                className = "flex-1 bg-gray-200 text-gray-700 py-2 rounded-lg font-semibold" > Cancel < /button> < /
                                div > <
                                /form> < /
                                div >
                            )
